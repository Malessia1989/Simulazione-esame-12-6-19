...
import it.polito.tdp.food.model.ArchiePeso;
...
public class FoodDao {
				...
				try {
					if(idMap.get(res.getInt("condiment_id"))==null) {
						...
						Condiment c= new Condiment(res.getInt("condiment_id"),
								res.getInt("food_code"), res.getString("display_name"),
	...
	public List<ArchiePeso> getArchiePeso(String numero, Map<Integer, Condiment> idMap) {
		...
		List<ArchiePeso> result= new ArrayList<ArchiePeso>();
		try {
			Connection conn = DBConnect.getConnection() ;

			PreparedStatement st = conn.prepareStatement(sql) ;
			st.setDouble(1,Double.parseDouble(numero));
			st.setDouble(2,Double.parseDouble(numero));
			...
			while(res.next()) {
				try {
					...
					if(c1!= null && c2!=null) {
						ArchiePeso a= new ArchiePeso(c1, c2, peso);
						result.add(a);
					}
